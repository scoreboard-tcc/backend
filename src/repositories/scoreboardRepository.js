...
class ScoreboardRepository {
  ...
  async findByAcademyIdWithMatches(academyId) {
    ...
    const data = await createQuery(tableName)
      .select('Scoreboard.id', 'Scoreboard.description', 'Match.id as matchId', 'Match.listed', 'Match.brokerTopic', 'Match.player1Name', 'Match.player2Name', knexInstance.raw(pin))
